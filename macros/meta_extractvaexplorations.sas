/**********************************************************************************************************************
 * Name:          meta_extractvaexplorations.sas
 *
 * Type:          Macro
 *
 * Description:   Extracts information from Metadata Server about Visual Analytics Exploration objects
 *                
 * Parameters:    Table           Name of table to be created 
 *                
 * Created:       10. Dec. 2015, Michael Larsen, SAS Institute
 * 
 **********************************************************************************************************************/
%macro meta_extractvaexplorations(Table=VAExplorations);
...
options linesize=256 nomlogic nomprint nonotes pagesize=1024 nosymbolgen;
  ...
  data &Table;
    ...
    ntran=metadata_getnobj("omsobj:DeployedComponent?@Id contains '.'",1,tran_uri);
    do c1=1 to ntran;
      rc=metadata_getnobj("omsobj:DeployedComponent?@Id contains '.'",c1,tran_uri);
      PublicType = "";
      rc=metadata_getattr(tran_uri,"PublicType",PublicType);
      if PublicType ne 'VisualExploration' 
        and PublicType ne 'VisualExplorationData'
        and PublicType ne 'VisualExplorationDataSource'
      then continue;
...
%mend;
*meta_extractvaexplorations(Table=Reports);
