...
/* --------------------------------------------------------------------------------------------------------- */
%macro object_spawner_logs(outtbl=,history=,config_location=,series=1);
...
%if %length(&config_location)=0 %then
	%do;
		%getfilelist(
			Path=&sas_config.&os_set_delim.ObjectSpawner&os_set_delim.Logs,
			Table=work._logs_object,
			Typefilter=log
			);
	%end;
...
/*------------------------------------------------------------------------------------------------------------*/
attrib  
	application length=$100 label='Application'
...
/*------------------------------------------------------------------------------------------------------------*/
			...
			RETAIN application dttm date time userid hostname;
			input;
/*------------------------------------------------------------------------------------------------------------*/
/* LOGNAME																								  	  */
/*------------------------------------------------------------------------------------------------------------*/
...
/* FIND																										  */
/*------------------------------------------------------------------------------------------------------------*/
/* The loop will look for all connections which have "APPNAME" in the string. When this is */
...
/* APPLICATION																								  */
/*------------------------------------------------------------------------------------------------------------*/
/* Extract from the string with = as the delimiter APPNAME=Enterprise Guide									  */
/*------------------------------------------------------------------------------------------------------------*/
					application=scan(_infile_,-1,'=');
/*------------------------------------------------------------------------------------------------------------*/
/* USERID																						  */
/*------------------------------------------------------------------------------------------------------------*/
/* Extract userid from the string. In large installations there may be more than one server that */
/* that BASE SAS sessions are started. This will allow all to be captured.									  */
/*------------------------------------------------------------------------------------------------------------*/
	userid = tranwrd(compress(scan(_infile_,17," "),'"'),".","");
				end;
/*------------------------------------------------------------------------------------------------------------*/
/* PID																						  */
/*------------------------------------------------------------------------------------------------------------*/
...
%if %length(&outtbl)=0 %then
	%do;
		%let outtbl=object_spawner_final;
	%end;
%else;
%do;
...
/*%object_spawner_logs;*/
/*%object_spawner_logs;*/
/*%object_spawner_logs(config_location=C:\SAS\Config\Lev1\ObjectSpawner\Logs\tmp);*/
/*%object_spawner_logs(history=1,outtbl=george);*/
/*%object_spawner_logs(outtbl=work.obspawn_data_log2);*/
