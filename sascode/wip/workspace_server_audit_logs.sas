...
/*%get_obscnt*/
/*%getfilelist*/
%macro workspace_Server_logs(outtbl=,cfg_path=);
	...
	data readlogs;
		...
		where membername like 'data_audit%'
			and Filesize > 0 and entrytype ="File";
	run;
	...
	%do i=1 %to %get_obscnt(readlogs);
		...
		data _audit_data_log&i.;
			...
			Hostname DateTime Userid Action Status Libref Engine Member MemberType Openmode Path $300.  Message logname sysin $100. fullpath logfile $200.;
			...
			PID=scan(scan("&&audit_load&i.",6,"_"),1,".","QR");
...
%workspace_Server_logs(outtbl=workspace_hsbc,cfg_path=C:\temp\hsbc\workspace);
